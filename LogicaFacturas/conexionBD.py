...
class ConexionBD:
    ...
    def eliminar_datos(self, consulta_sql, valores):
        """Elimina datos de la base de datos.

        :param consulta_sql: La consulta SQL de eliminación.
        :param valores: Los valores a utilizar en la consulta.
        """
        try:
            if self.conexion is None:
            ...
            if self.cursor is None:
                print("Error: No se ha creado el cursor.")
                return

            self.cursor.execute(consulta_sql, valores)
            self.conexion.commit()
            print("Datos eliminados correctamente de la base de datos.")
